...
//!
//! **Why it can pass while the Rust SDK is behind.** Some spec event types are
//! not yet implemented in this crate's `EventType`. Rather than hide that, the
//! gap is made explicit in `SPEC_EVENTS_NOT_YET_PORTED`: every entry there is a
//! spec event type this Rust SDK does not implement *yet*. The test asserts the
//! Rust enum equals `snapshot − not_yet_ported`, so it is green today and
//! documents exactly what is missing. When one of those events is implemented,
//! the enum gains it and this test fails until the entry is removed from the
//! list — turning "catch up to the spec" into a visible, enforced step instead
...
mod tests {
    ...
    const SPEC_EVENTS_NOT_YET_PORTED: &[&str] = &[
        // Reasoning events (events.ts). These deprecate the THINKING_* family
        // (slated for removal at spec 1.0.0); both currently coexist upstream.
        "REASONING_START",
